...
COMMENT ON COLUMN drug_rec.animal_id IS
    'animal id';

COMMENT ON COLUMN drug_rec.invoice_date IS
    'invoice date';
...
ALTER TABLE drug_rec
    ADD CONSTRAINT drug_rec_pk PRIMARY KEY ( animal_id,
                                             invoice_date,
                                             drug_id );
...
ALTER TABLE invoice ADD CONSTRAINT invoice_pk PRIMARY KEY ( animal_id,
                                                            invoice_date );
...
CREATE TABLE owner (
    ...
    owner_address       VARCHAR2(200) NOT NULL,
...
COMMENT ON COLUMN service_rec.animal_id IS
    'animal id';

COMMENT ON COLUMN service_rec.invoice_date IS
    'invoice date';

ALTER TABLE service_rec
    ADD CONSTRAINT service_rec_pk PRIMARY KEY ( service_code,
                                                invoice_date,
                                                animal_id );
...
CREATE TABLE vet (
    ...
    vet_foreign_visit_count  NUMBER(7)
);
...
COMMENT ON COLUMN vet.vet_foreign_visit_count IS
    'count of foreign visits';
...
ALTER TABLE drug_rec
    ADD CONSTRAINT invoice_drugrec FOREIGN KEY ( animal_id,
                                                 invoice_date )
        REFERENCES invoice ( animal_id,
                             invoice_date );

ALTER TABLE service_rec
    ADD CONSTRAINT invoice_servicerec FOREIGN KEY ( animal_id,
                                                    invoice_date )
        REFERENCES invoice ( animal_id,
                             invoice_date );
